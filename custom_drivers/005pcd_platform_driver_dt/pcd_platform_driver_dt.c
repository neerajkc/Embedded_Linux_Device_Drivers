...
struct pcdev_platform_data *pcdev_get_platdata_from_dt(struct device *dev)
{
	...
	if(of_property_read_string(dev_node, "nkc,device-serial-num", &pdata->serial_number))
	{
		dev_info(dev, "Missing serial number property\n");
		return ERR_PTR(-EINVAL);
	}
	
	if(of_property_read_u32(dev_node, "nkc,size", &pdata->size))
	{
		dev_info(dev, "Missing size property\n");
		return ERR_PTR(-EINVAL);
	}
	
	if(of_property_read_u32(dev_node, "nkc,perm", &pdata->perm))
	{
...
MODULE_DESCRIPTION("A pseudo character platform driver that handles n platform pcdevs");
